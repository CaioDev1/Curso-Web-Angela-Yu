...
userSchema.plugin(findOrCreate)       // CONFIGURA A COLLECTION A TER A FUNÇÃO findOrcreate USADA MAIS ABAIXO  
...
app.get('/secrets', function(req, res) {
    userCollection.find({secret: {$ne: null}}, function(err, doc) {
        if(err) {
            console.log(err)
        } else {
...
app.post('/submit', function(req, res) {
    ...
    userCollection.findById({_id: req.user.id}, function(err, doc) {
        if(err) {
            console.log(err)
        } else {
            doc.secret = secretText
